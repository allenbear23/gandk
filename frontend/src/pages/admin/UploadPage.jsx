...
export default function UploadPage() {
  ...
  const handleUpload = async (e) => {
    e.preventDefault();
    if (!file || !selectedSubject || !selectedUnitCode || !documentType) {
      alert("請填寫完整資訊並選擇檔案");
      return;
    }
    ...
    setUploading(true);
    try {
      await examAPI.uploadDocument(file, selectedSubject, selectedUnitCode, documentType);
      setFile(null);
      if (fileInputRef.current) fileInputRef.current.value = '';
      loadDocs(); // Reload document list
    } catch (err) {
          ...
          <form onSubmit={handleUpload} className="space-y-5">
            ...
            <div>
              ...
              <div className="flex bg-slate-100 p-1 rounded-lg">
                <button type="button" onClick={() => setDocumentType('textbook')} className={`flex-1 py-1.5 text-sm rounded-md font-medium ${documentType === 'textbook' ? 'bg-white shadow text-blue-700' : 'text-slate-500'}`}>課本內容</button>
